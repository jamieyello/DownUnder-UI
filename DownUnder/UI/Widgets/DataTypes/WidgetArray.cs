...
//        public Point Dimensions => Count != 0 && _widgets[0].Count != 0 ? new Point(_widgets.Count, _widgets[0].Count) : new Point();
...
//                for (int y = 0; y < widget_array[x].Count; x++) _widgets[x][y] = widget_array[x][y];
//            }

//            Count = _widgets.Count;
//        }
//        public WidgetArray(List<Widget> widgets, int x, int y)
//        {
//            if ((float)widgets.Count / x % 1f != 0) throw new Exception("Cannot create uniform array from given parameters.");
//            if (y != widgets.Count / x) throw new Exception("Cannot create uniform array from given parameters.");
//            int i = 0;
...
//            for (int x = 0; x < _widgets.Count; x++) GetColumn(x).AutoSizeWidth();
//            for (int y = 0; y < _widgets[0].Count; y++) GetRow(y).AutoSizeHeight();
//        }
...
//        public int IndexOf(WidgetList item) => _widgets.IndexOf(item);
...
//        public bool Contains(WidgetList item) => _widgets.Contains(item);
        
//        public WidgetList GetColumn(int x) => new WidgetList(_widgets[x]);
        
//        public WidgetList GetRow(int y) {
//            WidgetList result = new WidgetList();

//            Point2 dimensions = Dimensions;
//            for (int x = 0; x < dimensions.X; x++) result.Add(_widgets[x][y]);
...
//        public IEnumerator<WidgetList> GetEnumerator() => ((IList<WidgetList>)_widgets).GetEnumerator();
        
//        IEnumerator IEnumerable.GetEnumerator() => ((IList<WidgetList>)_widgets).GetEnumerator();
