...
namespace DownUnder.UI.Widgets
{
    ...
    public sealed partial class Widget : IParent, IDisposable, ICloneable, IEnumerable<Widget>, IWidget
    {
        ...
        /// <summary> When set to true this <see cref="Widget"/> will have no maximum size. WARNING: May cause crashing when used with visual <see cref="WidgetBehavior"/>s using render targets. </summary>
        public bool DisableMaxSize = false;
        ...
        public RectangleF Area
        {
            ...
            set
            {
                value = value.WithMaximumSize(MaximumSize);
                value = value.WithMinimumSize(MinimumSize);
                ...
                if (response.Override != null)
                {
                    ...
                    _area_backing = _area_backing.WithMaximumSize(MaximumSize);
                    if (_area_backing == previous_area) return;
                }
        ...
        public Point2 MaximumSize {
            get => DisableMaxSize ? new Point2(100000, 100000) : _maximum_size_backing;
            set {
                if (value == _maximum_size_backing) 
                    return;

                if (value.X > MAXIMUM_WIDGET_SIZE) throw new Exception($"{nameof(MaximumSize)} width cannot exceed {nameof(MAXIMUM_WIDGET_SIZE)} ({MAXIMUM_WIDGET_SIZE}).");
                if (value.Y > MAXIMUM_WIDGET_SIZE) throw new Exception($"{nameof(MaximumSize)} height cannot exceed {nameof(MAXIMUM_WIDGET_SIZE)} ({MAXIMUM_WIDGET_SIZE}).");
                ...
                if (args.Override != null) _maximum_size_backing = args.Override.Value;
                if (_maximum_size_backing == _previous_maximum_size) return;
                if (Area.WithMaximumSize(value) != Area)  Area = Area.WithMaximumSize(value);
        ...
        public Widget Clone() {
            ...
            c.DisableMaxSize = DisableMaxSize;
